...
#
	...
	if {$valid_mem_list != ""} {
		foreach element $valid_mem_list {
			...
			lappend modified_mem_list [mem_node_label_update $element]
		}
    ...
    # Compute the memory node label for a given hsi mem-range element so it
    # matches the label generated by the corresponding memory driver.
    # ddrpsv-family controllers emit "${cell}_ddr_memory" or "${cell}_hbm_memory"
    # All other memory IPs use the default "${cell}_memory".
    proc mem_node_label_update {element} {
        set cell [hsi::get_cells -hier $element]
        if {$cell != ""} {
            set ip_name [hsi get_property IP_NAME $cell]
        }
        set ddrpsv_ips [list "ddrpsv" "ddrcps" "lpddrmc" "axi_noc" "axi_noc2" "noc_mc_ddr5"]
        if {[lsearch -exact $ddrpsv_ips $ip_name] != -1} {
            set addr_block [hsi get_property ADDRESS_BLOCK $element]
            if {[string match -nocase "*HBM*" $addr_block]} {
                return "${element}_hbm_memory"
            }
            return "${element}_ddr_memory"
        }
        return "${element}_memory"
    }
