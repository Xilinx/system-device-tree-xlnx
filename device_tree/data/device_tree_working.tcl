	...
	foreach procc $proclist {
		...
		if {[lsearch $val_proclist $ip_name] >= 0 && $index == 0} {
			...
			namespace import ::${drvname}::\*
    			...
    			add_skeleton
		#	init_driver_param
    			foreach drv_handle [get_drivers 1] {
				#puts "handle $drv_handle"
				set ipname [get_property IP_TYPE [hsi::get_cells -hier $drv_handle]]
				if {[string match -nocase $ipname "processor"]} {
					continue
				}
				if {[string match -nocase [get_property IP_NAME [hsi::get_cells -hier $drv_handle]] "versal_cips"]} {
					continue
				}
				if {[string match -nocase [get_property IP_NAME [hsi::get_cells -hier $drv_handle]] "noc_nmu"]} {
					continue
				}
				if {[string match -nocase [get_property IP_NAME [hsi::get_cells -hier $drv_handle]] "noc_nsu"]} {
					continue
				}
				if {[string match -nocase [get_property IP_TYPE [hsi::get_cells -hier $drv_handle]] "MONITOR"]} {
					continue
				}
				if {[string match -nocase [get_property IP_TYPE [hsi::get_cells -hier $drv_handle]] "BUS"]} {
					continue
				}
				#puts "gen"
				...
				if {[string match -nocase $drv_handle "ai_engine_0"]} {
					set drv_file "$path/${driver_name}/data/${driver_name}.tcl"
					source -notrace $drv_file
					namespace import ::${driver_name}::\*
					::${driver_name}::generate $drv_handle
					namespace forget ::
				} else {
					continue
				}
		...
		if {[string match -nocase $proctype "psv_cortexa72"] || [string match -nocase $proctype "psv_cortexr5"] || [string match -nocase $proctype "psv_cortexa72"]} {
			foreach block_name $interface_block_names {
				if {[string match "C0_DDR_LOW0*" $block_name] || [string match "C1_DDR_LOW0*" $block_name]} {
					if {$is_ddr_low_0 == 0} {
						set base_value_0 [common::get_property BASE_VALUE [lindex [hsi::get_mem_ranges -of_objects [hsi::get_cells -hier $sw_proc] $periph] $i]]
					}
					set high_value_0 [common::get_property HIGH_VALUE [lindex [hsi::get_mem_ranges -of_objects [hsi::get_cells -hier $sw_proc] $periph] $i]]
					set is_ddr_low_0 1
				} elseif {[string match "C0_DDR_LOW1*" $block_name] || [string match "C1_DDR_LOW1*" $block_name]} {
					if {$is_ddr_low_1 == 0} {
						set base_value_1 [common::get_property BASE_VALUE [lindex [hsi::get_mem_ranges -of_objects [hsi::get_cells -hier $sw_proc] $periph] $i]]
					}
					set high_value_1 [common::get_property HIGH_VALUE [lindex [hsi::get_mem_ranges -of_objects [hsi::get_cells -hier $sw_proc] $periph] $i]]
					set is_ddr_low_1 1
				} elseif {[string match "C0_DDR_LOW2*" $block_name] || [string match "C1_DDR_LOW2*" $block_name]} {
					if {$is_ddr_low_2 == 0} {
						set base_value_2 [common::get_property BASE_VALUE [lindex [hsi::get_mem_ranges -of_objects [hsi::get_cells -hier $sw_proc] $periph] $i]]
					}
					set high_value_2 [common::get_property HIGH_VALUE [lindex [hsi::get_mem_ranges -of_objects [hsi::get_cells -hier $sw_proc] $periph] $i]]
					set is_ddr_low_2 1
				} elseif {[string match "C0_DDR_LOW3*" $block_name] || [string match "C1_DDR_LOW3*" $block_name]} {
					if {$is_ddr_low_3 == "0"} {
						set base_value_3 [common::get_property BASE_VALUE [lindex [hsi::get_mem_ranges -of_objects [hsi::get_cells -hier $sw_proc] $periph] $i]]
					}
					set high_value_3 [common::get_property HIGH_VALUE [lindex [hsi::get_mem_ranges -of_objects [hsi::get_cells -hier $sw_proc] $periph] $i]]
					set is_ddr_low_3 1
				} elseif {[string match "C0_DDR_CH1*" $block_name]} {
					if {$is_ddr_ch_1 == "0"} {
						set base_value_4 [common::get_property BASE_VALUE [lindex [hsi::get_mem_ranges -of_objects [hsi::get_cells -hier $sw_proc] $periph] $i]]
					}
					set high_value_4 [common::get_property HIGH_VALUE [lindex [hsi::get_mem_ranges -of_objects [hsi::get_cells -hier $sw_proc] $periph] $i]]
					set is_ddr_ch_1 1
				} elseif {[string match "C0_DDR_CH2*" $block_name]} {
					if {$is_ddr_ch_2 == "0"} {
				                set base_value_5 [common::get_property BASE_VALUE [lindex [hsi::get_mem_ranges -of_objects [hsi::get_cells -hier $sw_proc] $periph] $i]]
				        }
				        set high_value_5 [common::get_property HIGH_VALUE [lindex [hsi::get_mem_ranges -of_objects [hsi::get_cells -hier $sw_proc] $periph] $i]]
				        set is_ddr_ch_2 1
				} elseif {[string match "C0_DDR_CH3*" $block_name]} {
					 if {$is_ddr_ch_3 == "0"} {
				                set base_value_6 [common::get_property BASE_VALUE [lindex [hsi::get_mem_ranges -of_objects [hsi::get_cells -hier $sw_proc] $periph] $i]]
				        }
				        set high_value_6 [common::get_property HIGH_VALUE [lindex [hsi::get_mem_ranges -of_objects [hsi::get_cells -hier $sw_proc] $periph] $i]]
				        set is_ddr_ch_3 1
				}
				incr i
			}
 ...
 #   	hsi::utils::add_new_dts_param $mem_node "#size-cells" "0x2" hexint
  #  	hsi::utils::add_new_dts_param $mem_node ranges "" boolean
    	...
    	if {[string match -nocase $proctype "psv_cortexa72"] || [string match -nocase $proctype "psv_cortexr5"] || [string match -nocase $proctype "psv_pmc"]} {
			...
			add_prop $child_node reg $a53map hexint $default_dts
#        		hsi::utils::add_new_dts_param $child_node reg $a53map hexint
		}
    		if {![string_is_empty $r5map]} {
		        set child_node [create_node -l "memory_r5" -n "memory_r5" -d ${default_dts} -p $mem_node]
			add_prop $child_node reg $r5map hexint $default_dts
#        		hsi::utils::add_new_dts_param $child_node reg $r5map hexint
			...
			add_prop $child_node reg $pmumap hexint $default_dts
...
proc gen_cpu_cluster {os_handle} {
	...
	set default_dts "system-top.dts"
    	if {[string match -nocase $proctype "zynqmp"] || [string match -nocase $proctype "zynquplus"]} {
        	set cpu_node [create_node -n "cpus_a53" -l "cpus_a53" -d ${default_dts} -p root]
...
#		puts "0 [lindex $value 0] 1 [lindex $value 1] 2 [lindex $value 2]"
