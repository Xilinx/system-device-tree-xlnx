...
#	puts "treename $treeobj"
	...
	#		puts "already has childs $childnodes and node $node_name"
			...
			set interconnect [$treeobj insert root end "$node_name: $node_label@$node_unit_addr"]
			return $interconnect
		}
	}
...
#		puts "if adding $node_name: $node_label@$node_unit_addr to $parent_obj tree $treeobj"
		set drvnode [$treeobj insert $parent_obj end "$node_name: $node_label@$node_unit_addr"]	
		#puts "drvnode $drvnode"
	} else {
#		puts "2"
		foreach childnodes $main_childs {
			if {[string match -nocase $node_name "misc_clk_0"]} {
			puts "chl $childnodes nodename $node_name label $node_label"
			}
			if {[catch {set temp [[string match -nocase $childnodes "$node_name: $node_label@$node_unit_addr"]]} msg]} {
              	          if {[string match -nocase $childnodes "$node_name: $node_label"]} {
			if {[string match -nocase $node_name "misc_clk_0"]} {
				puts "matched $childnodes and $node_name: $node_label"
				}
#				puts "return $childnodes"
                                return $childnodes
			} else {
			puts "return1 $childnodes"
			if {[catch {set temp [string match -nocase $node_unit_addr]} msg]} {
                		set drvnode [$treeobj insert $parent_obj end "$node_name: $node_label@"]
			} else {
                		set drvnode [$treeobj insert $parent_obj end "$node_name: $node_label@$node_unit_addr"]
			}
			return $drvnode
...
foreach line $lines {
	...
	if {[string match *\\\{* $line]} {
		...
		#	puts "insert $line to $parent and [line_to_node_val $line] "
			...
			puts "node $node_name lable $node_label addr $node_unit_addr parent $parent file $tempfile"
	...
	#		puts "bool $line"
			...
			#			puts "append $chain_list and prop $next_prop"
				...
				if {[llength $spl] > 1 && [regexp -all {[\,]} $multival matched]} {
			...
			puts "========settting prop $prop and val $dtval for $parent========"
...
proc get_intr_id {drv_handle intr_port_name} {
	...
	foreach pin ${intr_port_name} {
		...
		if {[string match -nocase $proctype "psu_cortexa53"] || [string match -nocase $proctype "psv_cortexa72"]} {
			...
			if {[string match -nocase [get_property IP_NAME [hsi::get_cells -hier [get_sw_processor]]] "psu_cortexa53"] && [string match -nocase $intc "axi_intc"] } {
				set intc [hsi::utils::get_interrupt_parent $drv_handle $pin]
			}
			if {[string match -nocase [get_property IP_NAME [hsi::get_cells -hier [get_sw_processor]]] "psv_cortexa72"] && [string match -nocase $intc "axi_intc"] } {
				set intc [hsi::utils::get_interrupt_parent $drv_handle $pin]
			}
		}
		if {[string match -nocase $proctype "psu_cortexa53"] || [string match -nocase $proctype "psv_cortexa72"]} {
...
#	set proctype [get_property IP_NAME [hsi::get_cells -hier [get_sw_processor]]]
	...
	if {[string match -nocase $proctype "psv_cortexa72"] ||
		...
		dict set def_ps_mapping f1020000 label gpio
		dict set def_ps_mapping ff020000 label i2c0
		dict set def_ps_mapping ff030000 label i2c1
		dict set def_ps_mapping f1000000 label i2c2
		...
		dict set def_ps_mapping f1010000 label spi2
		dict set def_ps_mapping f1030000 label pmcqspi
		...
		dict set def_ps_mapping fca10000 label pcie
		dict set def_ps_mapping ff060000 label can0
		dict set def_ps_mapping ff070000 label can1
		dict set def_ps_mapping f1270000 label sysmon
...
proc zynq_gen_pl_clk_binding {drv_handle} {
	...
	#set file "$path/${drvname}/data/config.yaml"
	puts "==============> file $common_file"
	set mainline_ker [get_driver_config $common_file mainline_ker]
...
#	}
	...
	set dev_type ""
	if {[string_is_empty $dev_type] == 1} {
		set label [get_property IP_NAME [hsi::get_cell -hier $ip]]
	}
#	set proc_type [get_sw_proc_prop IP_NAME]
...
#		}
	...
	#	set bus_node ""
		...
		if {$status_disabled} {
			...
			puts "ADD it for $drv_handle and dts $default_dts"
		...
		if {[string match -nocase $ip_type "tsn_endpoint_ethernet_mac"]} {
			...
			set rt_node [create_node -n ${drv_handle} -l ${label} -u ${unit_addr} -d ${default_dts} -p $bus_node]
			#puts "rtnode $rt_node  devtype $dev_type label $label addr $unit_addr"
		#	set rt_node [add_or_get_dt_node -n ${dev_type} -l ${label} -u ${unit_addr} -d ${default_dts} -p $bus_node -auto_ref_parent]
		}
		add_prop $rt_node "status" "okay" string $default_dts
	#	hsi::utils::add_new_dts_param "${rt_node}" "status" "okay" string
...
#	set dev_type [get_property CONFIG.dev_type $processor]
	...
	set default_dts [set_drv_def_dts $drv_handle]
	set processor_type [get_property IP_NAME [hsi::get_cell -hier ${drv_handle}]]
	...
	foreach cpu ${processor_list} {
		...
		if {[string match -nocase $processor_type "psu_pmu"]} {
			set cpu_node [add_or_get_dt_node -n ${dev_type} -l "cpu4" -d ${default_dts} -p /]
			...
			set cpu_node [create_node -n ${dev_type} -l "cpu${cpu_nr}" -d ${default_dts} -p root]
